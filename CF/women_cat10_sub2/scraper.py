...
class BoutiqaatScraper:
    ...
    def _make_request_with_js(self, url: str) -> Optional[BeautifulSoup]:
        try:
            ...
            with sync_playwright() as p:
                ...
                if '/l/' in url or '/c/' in url:
                    try:
                        page.wait_for_selector('div.single-product-wrap', timeout=30000)
                        has_products = True
                    except:
                        logger.warning(f"No products found on page (empty category): {url}")
                        has_products = False
    ...
    def get_products(self, category_url: str) -> List[Dict]:
        ...
        products = self._extract_all_products(soup, category_url)
        if not products:
            logger.info(f"Category page loaded successfully but contains no products")
        return products
