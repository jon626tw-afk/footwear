...
# Hardcoded subcategory URLs for Footwear Group 3
# Note: kids-footwear URL ends with /c/ instead of /l/
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/women/footwear/boots-1/l/",
    "https://www.boutiqaat.com/ar-kw/women/kids-footwear/c/",
    "https://www.boutiqaat.com/ar-kw/women/footwear/shoe-care-accessories-1/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/footwear/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/footwear/excel-files"
            )
